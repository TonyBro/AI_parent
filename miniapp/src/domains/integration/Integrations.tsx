...
import { getWorkingHours, patchWorkingHours } from "../action/api";
import type { WorkingHourDto } from "../action/types";
import { 
  Section, 
  ListItem, 
  ToggleItem, 
        ...
        if (availableInt) {
          ...
          if (integration.settings && typeof integration.settings === "object") {
            ...
            setApiSettings({
              ...
              dynamicParams: settings.dynamicParams || "",
            });
      ...
      if (activeIntegration.type.type === "custom_api") {
        return (
          <div>
            ...
            <Section title={t("miniapp.dynamic_parameters_instructions")}>
              <FormTextArea
                placeholder={t("miniapp.dynamic_parameters_placeholder")}
                value={apiSettings.dynamicParams}
                onChange={(e) => {
                  setApiSettings({ ...apiSettings, dynamicParams: e.target.value });
                  setTestResult(null);
                  setHasTestedSuccessfully(false);
                }}
                rows={4}
            ...
            <HintText style={{ padding: '0 16px', fontSize: '13px', marginTop: '-8px', marginBottom: '16px' }}>
              {t("miniapp.dynamic_parameters_hint")}
            </HintText>
