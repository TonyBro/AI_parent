...
export function Broadcast({ 
  ...
  useEffect(() => {
    (async () => {
      try {
        const [bs, qs] = await Promise.all([getBroadcastSettings(botId), getQuizSets(botId)]);
        setBroadcasts(bs);
        setQuizSets(qs);
        if (qs.length > 0) {
  ...
  const toggleQuizSet = (id: string) => {
    setNowForm((f) => ({
      ...f,
      quiz_set_ids: f.quiz_set_ids.includes(id) ? f.quiz_set_ids.filter((x) => x !== id) : [...f.quiz_set_ids, id],
    }));
  };
            ...
            {broadcasts.length === 0 ? (
              ...
              broadcasts.map(b => (
                <ListItem
                  ...
                  subtitle={`${String(b.send_time_hour).padStart(2, "0")}:00 UTC — ${b.enabled ? t("miniapp.status_active") : t("miniapp.status_disabled")}`}
        ...
        <Section title={t("miniapp.broadcast_now_ai_select_quizzes")}>
          {quizSets.length === 0 ? (
            ...
            quizSets.map((qs) => (
              <ToggleItem
                key={qs.id}
                checked={settings.quiz_set_ids?.includes(qs.id) || false}
                onChange={(val) => {
                  const current = settings.quiz_set_ids || [];
                  const next = val ? [...current, qs.id] : current.filter(id => id !== qs.id);
          ...
          <Section title={t("miniapp.broadcast_now_ai_select_quizzes")}>
            {quizSets.length === 0 ? (
              ...
              quizSets.map((qs) => (
                <ToggleItem
                  key={qs.id}
                  checked={nowForm.quiz_set_ids.includes(qs.id)}
                  onChange={() => toggleQuizSet(qs.id)}
                  label={qs.title}
                  subtitle={t("miniapp.questions_count", { count: qs.items?.length || 0 })}
                />
              ))
